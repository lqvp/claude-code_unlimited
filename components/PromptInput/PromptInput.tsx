...
function PromptInput({
  ...
  const expandPasteTimeoutRef = useRef<NodeJS.Timeout | undefined>(undefined);
  const lastTruncatedPasteRef = useRef<{ id: number; fullText: string } | null>(null);
  ...
  function onTextPaste(rawText: string) {
    ...
    if (text.length > PASTE_THRESHOLD || numLines > maxLines) {
      // Check if this is a double-paste to expand a previously truncated paste
      if (lastTruncatedPasteRef.current && showExpandPasteHint) {
        // Double-paste detected: expand the previous truncated paste
        const { id: prevId, fullText } = lastTruncatedPasteRef.current;
        lastTruncatedPasteRef.current = null;
        setShowExpandPasteHint(false);
        if (expandPasteTimeoutRef.current) {
          clearTimeout(expandPasteTimeoutRef.current);
          expandPasteTimeoutRef.current = undefined;
        }
        // Replace the truncated ref in input with the full text
        const ref = formatPastedTextRef(prevId, getPastedTextRefNumLines(fullText));
        const refIndex = input.indexOf(ref);
        if (refIndex !== -1) {
          pushToBuffer(input, cursorOffset, pastedContents);
          const newInput = input.slice(0, refIndex) + fullText + input.slice(refIndex + ref.length);
          trackAndSetInput(newInput);
          setCursorOffset(refIndex + fullText.length);
          // Remove from pastedContents since it's now inline
          setPastedContents(prev => {
            const next = { ...prev };
            delete next[prevId];
            return next;
      ...
      // Track this truncated paste for potential double-paste expansion
      lastTruncatedPasteRef.current = { id: pasteId, fullText: text };
      setShowExpandPasteHint(true);
      if (expandPasteTimeoutRef.current) {
        clearTimeout(expandPasteTimeoutRef.current);
      }
      expandPasteTimeoutRef.current = setTimeout(() => {
        setShowExpandPasteHint(false);
        lastTruncatedPasteRef.current = null;
        expandPasteTimeoutRef.current = undefined;
      }, 800);
